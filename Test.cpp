...
TEST_CASE("check number of cards"){
    int number_of_cards=52;
    Card card(number_of_cards);
    CHECK(card.cards_numbers()==52);
}
TEST_CASE("check the constructor class Player"){
    Player p("alice");
    CHECK_NOTHROW(p);
}
TEST_CASE("check function getName in class player"){
    Player p("alice");
    CHECK(p.getName().compare("alice")==0);
}
